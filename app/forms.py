...
class InsuranceForm(FlaskForm):
    coverage_type = SelectField(
        "Coverage Type",
        choices=[("Medical_insurance", "Medical Insurance"), ("Accident", "Accident")],
        validators=[DataRequired()],
    )
    Insurance_company = SelectField(
        "Insurance Company",
        choices=[("sanlam", "Sanlam"), ("RADIANT", "Radiant")],
        validators=[DataRequired()],
    )
    premium_amount = IntegerField(
        "Premium Amount", validators=[DataRequired(), NumberRange(min=0)]
    )
...
class RegistrationForm(FlaskForm):
    ...
    UserRole = SelectField(
        "User Role", choices=[("admin", "Admin"), ("user", "User")], default="user"
    )
...
class CarForm(FlaskForm):
    ...
    rental_price_per_day = IntegerField(
        "Rental Price per Day", validators=[DataRequired(), NumberRange(min=0)]
    )
    insurance_id = IntegerField(
        "Insurance ID", validators=[DataRequired(), NumberRange(min=1)]
    )
...
class CustomerForm(FlaskForm):
    ...
    contact_information = StringField(
        "Contact Information", validators=[DataRequired()]
    )
    Email = StringField("Email", validators=[DataRequired(), Email()])
    address = StringField("Address", validators=[DataRequired()])
    driving_license_number = StringField(
        "Driving License Number", validators=[DataRequired()]
    )


class LocationForm(FlaskForm):
    location_ID = StringField(
        "Location ID", validators=[DataRequired(), Length(max=10)]
    )
    address = StringField("Address", validators=[DataRequired(), Length(max=50)])
    contact_information = StringField(
        "Contact Information", validators=[DataRequired(), Length(max=20)]
    )
    operating_hours = StringField(
        "Operating Hours", validators=[DataRequired(), Length(max=50)]
    )
...
class EmployeeForm(FlaskForm):
    ...
    First_name = StringField(
        "First Name", validators=[InputRequired(), Length(max=250)]
    )
    ...
    TelephoneNumber = StringField(
        "Telephone Number", validators=[InputRequired(), Length(max=20)]
    )
    ...
    location_id = StringField(
        "Location ID", validators=[InputRequired(), Length(max=10)]
    )
...
class RentalForm(FlaskForm):
    ...
    total_rental_cost = IntegerField(
        "Total Rental Cost", validators=[NumberRange(min=0)]
    )
...
class PaymentForm(FlaskForm):
    payment_date = DateField(
        "Payment Date", format="%Y-%m-%d", validators=[DataRequired()]
    )
    ...
    rental_id = IntegerField(
        "Rental ID", validators=[DataRequired(), NumberRange(min=1)]
    )
    ...
    cvv = StringField(
        ...
        validators=[
            ...
            Regexp("^[0-9]*$", message="CVV must be numeric"),
        ],
    )
    expiry_date = DateField(
        "Expiry Date", format="%Y-%m-%d", validators=[DataRequired()]
    )
...
class MaintenanceForm(FlaskForm):
    maintenance_type = StringField(
        "Maintenance Type", validators=[DataRequired(), Length(max=200)]
    )
    maintenance_date = DateField(
        "Maintenance Date", format="%Y-%m-%d", validators=[DataRequired()]
    )
    car_id = SelectField("Car", coerce=str, validators=[DataRequired()])
    maintenance_cost = IntegerField(
        "Maintenance Cost", validators=[DataRequired(), NumberRange(min=0)]
    )
